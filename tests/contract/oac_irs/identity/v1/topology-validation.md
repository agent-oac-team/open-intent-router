...
- OAC Next.js Knowledge Route 使用 Node runtime，可用标准 `node:crypto` 在服务端签名，不需要浏览器持有签名密钥。
...
- Coze 使用独立 `coze_workflow` 服务凭证，只允许只读 Knowledge；不得持有 OAC user/admin 的签名密钥。
...
| OAC Next Knowledge Route | 服务端调用 Go `/api/auth/me` 得到的 user/role | Next Node Route | 按 local/test 配置 | `oac_admin` |
| Coze Workflow | 独立服务凭证映射 | Adapter credential verifier | test | `coze_workflow` |

## 后续实现约束

签名实现放在 OAC Host Adapter identity 模块和 OAC/Next 对应 signer 中，不进入 OIR Core Schema、Prompt 或配置。真实密钥只通过环境变量或 secret store 注入，日志只允许记录 key ID、audience、认证结果和脱敏 principal reference。
