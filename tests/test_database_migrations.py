...
def test_postgresql_schema_contains_native_definition_cutover_support() -> None:
    ...
    assert "ADD COLUMN IF NOT EXISTS active BOOLEAN DEFAULT false NOT NULL" in schema_sql
    assert "ADD COLUMN IF NOT EXISTS target_fingerprint VARCHAR(64)" in schema_sql
    assert "__native_definition_global__" in schema_sql
