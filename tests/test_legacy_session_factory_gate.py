...
from scripts.check_legacy_session_factory_usage import find_call_sites, unexpected_call_sites
...
def test_legacy_session_factory_gate_detects_module_aliases(tmp_path) -> None:
    ...
    assert find_call_sites(tmp_path) == frozenset({"app/sneaky_factory.py:3"})
