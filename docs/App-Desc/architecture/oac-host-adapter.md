...
`host_apps/oac` 负责组装进程，`host_adapters/oac` 负责 IRS 兼容协议、身份、映射、Fallback、Shadow 和 Cutover。Adapter 只能调用 `app.application` 暴露的 OIR 应用端口。`app/` 不得反向导入 Host，也不得公开 OAC、IRS、Coze、飞书、`bot_id` 或 `route_path` 专有字段。
...
- `GET /capabilities`：仅输出版本、模式、依赖健康、Write Fence、Circuit 和脱敏签名门禁状态。
- OIR Native API 挂载于 `/oir/api/v1`，Legacy `/api/v1` 不按 Body 猜测协议。

## Identity

首期强制 `tenant_id=oac`。OAC user、OAC Admin 和 Coze Knowledge 使用不同 key/credential class。Adapter 校验 key ID、audience、timestamp、nonce、正文哈希和签名。Coze 只有 Knowledge read 权限，不能调用 Registry、Knowledge Admin 或 Runtime write。
...
- Route 只有在提交状态为 `not_accepted` 时可自动回退 IRS。
- 只读 Knowledge 可按 Circuit policy 回退。
- 超时且提交未知时 fail closed。
- Registry、Knowledge Admin、Event、Plan 和 Memory 写入禁止回退或双写。
...
`GET /capabilities` 返回 Adapter/Core/Schema/Policy 版本，Knowledge/Memory/Shadow/Fallback 模式，Registry 健康，Write Fence、Circuit 状态，以及 current=`v2`、accepted=`[v2]`、V1 compatibility disabled、profile/Bundle catalog 健康和按版本/类别/操作/终态聚合的低基数计数。响应不返回 Principal claims、subject、key ID、entitlement 持有情况、数据库 URL、fallback URL、密钥、Token、签名、正文或 Ticket。
