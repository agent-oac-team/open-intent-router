...
1. 提供一套可本地启动的中控后端服务。
2. 提供一套可视化测试台，用于演示子牙如何调用中控、配置 Agent、观察路由结果。
3. 提供一组子牙风格的演示 Agent 和固定演示问题。
4. 提供 API、配置、部署和验证说明，便于评估后续进入子牙研发迭代。
...
| Agent 候选过滤 | 已具备 | 权限过滤是硬边界；标签/语义筛选当前只做召回观察，不裁剪候选 |
| 多意图识别 | 已具备 | 支持多意图执行计划生成、确认、执行、恢复和取消 |
| Evidence Provider | 已具备轻量接口 | 当前提供文件型固定问题命中示例 |
...
| 子牙 Knowledge Source 适配 | P1 | 将子牙知识库作为 Evidence Provider 接入 |
| 评估与回放 | P1 | 建立标准测试集、路由准确率和长对话回归 |
| 子牙 Adapter | P0 | 负责用户、权限、Agent 配置、页面跳转和知识库接口映射 |
...
4. 子牙专有字段通过 Adapter 或配置映射，不进入中控核心代码。
...
flowchart TD
    ...
    subgraph Z["子牙宿主系统"]
        ...
        O["打开或切换目标 Agent / 页面"]
        X["退出当前 Agent 上下文"]
        Q["展示追问、直接回复<br/>计划确认或 UI Handoff"]
    ...
    subgraph R["中控服务 open-intent-router"]
        ...
        RR["RouteResponse<br/>continue / open / exit / clarify / plan"]
    end
    ...
    D -->|open_agent| O --> AG
    D -->|exit_agent| X --> U
    D -->|clarify / reply / plan| Q --> U
    AG --> AR --> V --> U
```

这里的约定是：每轮用户输入都先走 `/route`，再由子牙根据中控返回的动作决定继续当前子 Agent、切换 Agent、退出上下文、展示追问或执行 UI Handoff。图中不展开 `route-and-invoke`、`route-and-execute` 和 Agent Event，避免把演示便利接口误认为子牙生产主链路。Agent Event 适合在路由之后作为执行状态回传机制，用于同步子 Agent 的阶段结果、失败、阻塞或摘要。
...
4. Agent Event 暂不放在第一阶段主流程中；后续如果需要同步子 Agent 的阶段结果、失败、阻塞或摘要，再作为执行状态回传机制接入。
...
1. 中控以独立服务部署，还是嵌入子牙工程。
2. 测试环境使用 SQLite、PostgreSQL 还是子牙现有数据库。
3. 子牙 Agent 清单由配置文件、数据库还是内部平台注册表提供。
4. 子牙用户、角色、组织、租户字段如何映射。
5. 子牙页面跳转协议如何表达。
6. 子牙知识库是否提供 HTTP 检索接口。
7. 生产环境是否允许中控直接访问大模型服务。
8. 日志、审计、脱敏、密钥和网络策略由谁负责。
9. 第一批接入哪些 Agent，验收问题集由谁提供。
10. 下个月或下下个月进入哪个研发迭代窗口。

## 14. 后续开发路径

推荐顺序：

1. **交付评估阶段**：完成演示、部署说明、API 对齐和研发/IT 评估。
2. **子牙 Adapter 阶段**：完成用户、权限、Agent、页面和知识库接口映射。
3. **Context Pack 阶段**：实现上下文预算、裁剪、摘要压缩和可观察日志。
4. **Memory MVP 阶段**：实现 Session Memory、Task Memory 和可追踪召回。
5. **Knowledge Source 阶段**：接入子牙知识库 Evidence Provider。
6. **生产验证阶段**：补齐标准测试集、回归、压测、审计和上线方案。

当前最优先的下一步是约研发和 IT 做技术对齐，并以本测试台演示 `/route` 主链路。
